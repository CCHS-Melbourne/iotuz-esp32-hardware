...
class Tetris
{
  ...
  void run()
  {
    ...
    {
      tft2.fillRectangle(0, i, LCD_WIDTH, 2, 0x1f - i / c);
    }

    tft2.fillRectangle(BOARD_LEFT, 0, BOARD_RIGHT-BOARD_LEFT-1, BOARD_BOTTOM, BG_COLOR);
    ...
    do {
      ...
      if ( newShape )
      {
        ...
        // draw next box
        tft2.fillRectangle(30, 100, BLOCK_SIZE * 6, BLOCK_SIZE * 5, BLACK);
        tft2.drawRectangle(29, 99, BLOCK_SIZE * 6 + 1, BLOCK_SIZE * 5 + 1, WHITE);

        byte *shape = all_shapes[next[next_c]];
        for ( int i = 0; i < 4; i++ )
        {
          byte *block = shape + i * 2;
          tft2.fillRectangleUseBevel(
  ...
  void scoreBoard()
  {
    tft2.fillRectangle(6, 3, 128, 50, BLACK);
    tft2.drawString("Level", 8, 8, 2, YELLOW);
    tft2.drawString("Lines", 8, 32, 2, 0x3f);
    tft2.drawNumber(level, 74, 8, 2, YELLOW);
    tft2.drawNumber(lines, 74, 32, 2, 0x3f);
    tft2.drawRectangle(5, 2, 130, 52, 0xffff);
  }
